...
import (
	...
	"github.com/gepis/strge/pkg/promise"
...
// xattrs from fileconstants to a tar header
func ReadSecurityXattrToTarHeader(path string, hdr *tar.Header) error {
	...
	for _, xattr := range []string{"security.capability", "security.ima"} {
		capability, err := constants.Lgetxattr(path, xattr)
...
// ReadUserXattrToTarHeader reads user.* xattr from fileconstants to a tar header
func ReadUserXattrToTarHeader(path string, hdr *tar.Header) error {
	...
	for _, key := range xattrs {
		if strings.HasPrefix(key, "user.") {
			value, err := constants.Lgetxattr(path, key)
			if err != nil {
				if errors.Is(err, constants.E2BIG) {
...
func createTarFile(path, extractDir string, hdr *tar.Header, reader io.Reader, Lchown bool, chownOpts *idtools.IDPair, inUserns, ignoreChownErrors bool, forceMask *os.FileMode, buffer []byte) error {
	...
	switch hdr.Typeflag {
		...
		case tar.TypeReg, tar.TypeRegA:
			// Source is regular file. We use constants.OpenFileSequential to use sequential
	...
	if forceMask != nil && hdr.Typeflag != tar.TypeSymlink {
		...
		if err := constants.Lsetxattr(path, containersOverrideXattr, []byte(value), 0); err != nil {
			return err
		}
	}
	...
	if hdr.Typeflag == tar.TypeLink {
		if fi, err := os.Lstat(hdr.Linkname); err == nil && (fi.Mode()&os.ModeSymlink == 0) {
			if err := constants.Chtimes(path, aTime, hdr.ModTime); err != nil {
				return err
			}
		}
		...
		if err := constants.LUtimesNano(path, ts); err != nil && err != constants.ErrNotSupportedPlatform {
			return err
		}
	}
	...
	for key, value := range hdr.Xattrs {
		if err := constants.Lsetxattr(path, key, []byte(value), 0); err != nil {
	...
	if len(errs) > 0 {
		logrus.WithFields(logrus.Fields{
			"errors": errs,
		}).Warn("ignored xattrs in archive: underlying fileconstants doesn't support them")
	}
...
func TarWithOptions(srcPath string, options *TarOptions) (io.ReadCloser, error) {
	...
	go func() {
		...
		// mutating the fileconstants and we can see transient errors
		// from this
...
func Unpack(decompressedArchive io.Reader, dest string, options *TarOptions) error {
	...
	if options.ForceMask != nil {
		...
		if err == nil {
			...
			if err := constants.Lsetxattr(dest, containersOverrideXattr, []byte(value), 0); err != nil {
				return err
			}
		}
	}
...
loop:
	...
	for _, hdr := range dirs {
		...
		if err := constants.Chtimes(path, hdr.AccessTime, hdr.ModTime); err != nil {
			return err
		}
	}
...
// CopyWithTar creates a tar archive of fileconstants path `src`, and
// unpacks it at fileconstants path `dst`.
...
// at fileconstants path `src`, and unpacks it at `dst`.
...
// location in the container's fileconstants, mapping permissions using the
// container's ID maps
