...
func walkchunk(path string, fi os.FileInfo, dir string, root *FileInfo) error {
	...
	stat, err := constants.FromStatT(fi.Sys().(*syscall.Stat_t))
	if err != nil {
		return err
	}
	info.stat = stat
	info.capability, err = constants.Lgetxattr(cpath, "security.capability") // lgetxattr(2): fs access
	if err != nil && !errors.Is(err, constants.EOPNOTSUPP) {
		return err
	}
	xattrs, err := constants.Llistxattr(cpath)
	if err != nil && !errors.Is(err, constants.EOPNOTSUPP) {
		return err
	}
	for _, key := range xattrs {
		if strings.HasPrefix(key, "user.") {
			value, err := constants.Lgetxattr(cpath, key)
			if err != nil {
...
func overlayDeletedFile(layers []string, root, path string, fi os.FileInfo) (string, error) {
	...
	opaque, err := constants.Lgetxattr(filepath.Join(root, path), getOverlayOpaqueXattrName())
	if err != nil {
		return "", err
	}
