...
type Layer struct {
	...
	// succeeds to find the location of the container's root fileconstants.
...
type LayerStore interface {
	...
	// underlying context can accept a "size" option.  At this time, most
	// underlying context do not themselves distinguish between writeable
	...
	Mount(id string, options context.MountOpts) (string, error)
	...
	ApplyDiffFromStagingDirectory(id, stagingDirectory string, diffOutput *context.DriverWithDifferOutput, options *context.ApplyDiffOpts) error
	...
	PutAdditionalLayer(id string, parentLayer *Layer, names []string, aLayer context.AdditionalLayer) (layer *Layer, err error)
}
...
func (s *store) newLayerStore(rundir string, layerdir string, driver context.Driver) (LayerStore, error) {
...
func newROLayerStore(rundir string, layerdir string, driver context.Driver) (ROLayerStore, error) {
...
func (r *layerStore) PutAdditionalLayer(id string, parentLayer *Layer, names []string, aLayer context.AdditionalLayer) (layer *Layer, err error) {
...
func (r *layerStore) Mount(id string, options context.MountOpts) (string, error) {
...
func (r *layerStore) newFileGetter(id string) (context.FileGetCloser, error) {
	if getter, ok := r.driver.(context.DiffGetterDriver); ok {
		return getter.DiffGetter(id)
	}
	path, err := r.Mount(id, context.MountOpts{})
	if err != nil {
		return nil, err
	}
...
func (r *layerStore) Diff(from, to string, options *DiffOptions) (io.ReadCloser, error) {
	...
	if ad, ok := r.driver.(context.AdditionalLayerStoreDriver); ok {
...
func (r *layerStore) ApplyDiff(to string, diff io.Reader) (size int64, err error) {
	...
	idLogger, err := tarlogger.NewLogger(func(h *tar.Header) {
...
func (r *layerStore) DifferTarget(id string) (string, error) {
	ddriver, ok := r.driver.(context.DriverWithDiffer)
	if !ok {
...
func (r *layerStore) ApplyDiffFromStagingDirectory(id, stagingDirectory string, diffOutput *context.DriverWithDifferOutput, options *context.ApplyDiffOpts) error {
	ddriver, ok := r.driver.(context.DriverWithDiffer)
	if !ok {
		return ErrNotSupported
	}
...
func (r *layerStore) ApplyDiffWithDiffer(to string, options *context.ApplyDiffOpts, differ context.Differ) (*context.DriverWithDifferOutput, error) {
	ddriver, ok := r.driver.(context.DriverWithDiffer)
	if !ok {
