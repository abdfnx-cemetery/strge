...
func Init(home string, options context.Options) (context.Driver, error) {
	...
	d.updater = context.NewNaiveLayerIDMapUpdater(d)
	d.naiveDiff = context.NewNaiveDiffDriver(d, d.updater)

	return d, nil
}

// Driver holds information about the driver, home directory of the driver.
// Driver implements context.ProtoDriver. It uses only basic vfs operations.
// In order to support layering, files are copied from the parent layer into the new layer. There is no copy-on-write support.
// Driver must be wrapped in NaiveDiffDriver to be used as a context.Driver
type Driver struct {
	name              string
	homes             []string
	...
	updater           context.LayerIDMapUpdater
}
...
func (d *Driver) create(id, parent string, opts *context.CreateOpts, ro bool) (retErr error) {
	...
	if parent != "" {
		parentDir, err := d.Get(parent, context.MountOpts{})
		if err != nil {
