...
import (
	...
	"github.com/gepis/strge/context/overlayutils"
	"github.com/gepis/strge/context/quota"
	"github.com/gepis/strge/pkg/archive"
	"github.com/gepis/strge/pkg/chrootarchive"
	"github.com/gepis/strge/pkg/directory"
	"github.com/gepis/strge/pkg/fsutils"
...
func init() {
	context.Register("overlay", Init)
	context.Register("overlay2", Init)
}
...
func checkAndRecordOverlaySupport(fsMagic context.FsMagic, home, runhome string) (bool, error) {
...
func Init(home string, options context.Options) (context.Driver, error) {
	...
	if opts.mountProgram != "" {
		...
		switch fsMagic {
		case context.FsMagicAufs, context.FsMagicZfs, context.FsMagicOverlay, context.FsMagicEcryptfs:
			return nil, errors.Wrapf(context.ErrIncompatibleFS, "'overlay' is not supported over %s, a mount_program is required", backingFs)
		}
	}
	...
	fileSystemType := context.FsMagicOverlay
	if opts.mountProgram != "" {
		fileSystemType = context.FsMagicFUSE
	}
	...
	d := &Driver{
		...
		ctr:              context.NewRefCounter(context.NewFsChecker(fileSystemType)),
	...
	d.naiveDiff = context.NewNaiveDiffDriver(d, context.NewNaiveLayerIDMapUpdater(d))
	if backingFs == "xfs" {
...
func supportsOverlay(home string, homeMagic context.FsMagic, rootUID, rootGID int) (supportsDType bool, err error) {
	...
	if err == nil {
		...
		if len(flags) < unix.Getpagesize() {
			...
			if err == nil {
				...
				return supportsDType, errors.Wrap(context.ErrNotSupported, "kernel too old to provide multiple lowers feature for overlay")
			}
		...
		return supportsDType, errors.Wrapf(context.ErrIncompatibleFS, "'overlay' is not supported over %s at %q", backingFs, home)
	}
	...
	return supportsDType, errors.Wrap(context.ErrNotSupported, "'overlay' not found as a supported filesystem on this host. Please ensure kernel is new enough and has overlay support loaded.")
}
...
func (d *Driver) LookupAdditionalLayer(dgst digest.Digest, ref string) (context.AdditionalLayer, error) {
...
// This API is experimental and can be changed without bumping the major version number.
// TODO: to remove the comment once it's no longer experimental.
func (d *Driver) LookupAdditionalLayerByID(id string) (context.AdditionalLayer, error) {
...
func (d *Driver) Create(id, parent string, opts *context.CreateOpts) (retErr error) {
...
func (d *Driver) get(id string, disableShifting bool, options context.MountOpts) (_ string, retErr error) {
	d.locker.Lock(id)
	defer d.locker.Unlock(id)
	dir := d.dir(id)
	if _, err := os.Stat(dir); err != nil {
...
func (d *Driver) ApplyDiffWithDiffer(id, parent string, options *context.ApplyDiffOpts, differ context.Differ) (output context.DriverWithDifferOutput, err error) {
	...
	if id == "" {
		...
		if err != nil && !os.IsExist(err) {
			return context.DriverWithDifferOutput{}, err
		}
...
func (d *Driver) ApplyDiffFromStagingDirectory(id, parent, stagingDirectory string, diffOutput *context.DriverWithDifferOutput, options *context.ApplyDiffOpts) error {
...
func (d *Driver) UpdateLayerIDMap(id string, toContainer, toHost *idtools.IDMappings, mountLabel string) error {
	...
	err = context.ChownPathByMaps(layerFs, toContainer, toHost)
	if err != nil {
	...
	for _, ls := range d.options.layerStores {
		...
		for _, p := range []string{
			...
			if _, err := os.Stat(p); err != nil {
				return "", errors.Wrapf(context.ErrLayerUnknown,
