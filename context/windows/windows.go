...
import (
	...
	"github.com/gepis/strge/pkg/reexec"
...
func init() {
	context.Register("windowsfilter", InitFilter)
	...
	if os.Getenv("DOCKER_WINDOWSFILTER_NOREEXEC") != "" {
		logrus.Warnf("WindowsGepisContext is set to not re-exec. This is intended for debugging purposes only.")
		noreexec = true
	} else {
...
func InitFilter(home string, options context.Options) (context.Driver, error) {
	logrus.Debugf("WindowsGepisContext InitFilter at %s", home)
	...
	d := &Driver{
		...
		ctr:   context.NewRefCounter(&checker{}),
	}
	return d, nil
}
...
func panicIfUsedByLcow() {
	if constants.LCOWSupported() {
		panic("inconsistency - windowsfilter context should not be used when in LCOW mode")
	}
}
...
func (d *Driver) CreateFromTemplate(id, template string, templateIDMappings *idtools.IDMappings, parent string, parentIDMappings *idtools.IDMappings, opts *context.CreateOpts, readWrite bool) error {
	return context.NaiveCreateFromTemplate(d, id, template, templateIDMappings, parent, parentIDMappings, opts, readWrite)
}
...
func (d *Driver) Remove(id string) error {
	...
	retryCount := 0
	osv := constants.GetOSVersion()
	for {
		// Get and terminate any template VMs that are currently using the layer.
		// Note: It is unfortunate that we end up in the contexts Remove() call
...
func (d *Driver) Get(id string, options context.MountOpts) (string, error) {
	panicIfUsedByLcow()
	logrus.Debugf("WindowsGepisContext Get() id %s mountLabel %s", id, options.MountLabel)
	var dir string
...
func (d *Driver) Put(id string) error {
	panicIfUsedByLcow()
	logrus.Debugf("WindowsGepisContext Put() id %s", id)
...
// new layer in bytes.
// The layer should not be mounted when calling this function
func (d *Driver) ApplyDiff(id, parent string, options context.ApplyDiffOpts) (int64, error) {
	panicIfUsedByLcow()
	var layerChain []string
	if parent != "" {
		rPId, err := d.resolveID(parent)
...
func (d *Driver) DiffSize(id string, idMappings *idtools.IDMappings, parent string, parentMappings *idtools.IDMappings, mountLabel string) (size int64, err error) {
	...
	layerFs, err := d.Get(id, context.MountOpts{})
	if err != nil {
		return
	}
	defer d.Put(id)
