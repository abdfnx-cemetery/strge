...
import (
	...
	// register all of the built-in context
	_ "github.com/gepis/strge/context/register"
...
type Store interface {
	...
	// by the Store.
	GraphDriver() (context.Driver, error)
	...
	ApplyDiffWithDiffer(to string, options *context.ApplyDiffOpts, differ context.Differ) (*context.DriverWithDifferOutput, error)

	// ApplyDiffFromStagingDirectory uses stagingDirectory to create the diff.
	ApplyDiffFromStagingDirectory(to, stagingDirectory string, diffOutput *context.DriverWithDifferOutput, options *context.ApplyDiffOpts) error
...
func (s *store) getGraphDriver() (context.Driver, error) {
	...
	driver, err := context.New(s.graphDriverName, config)
	if err != nil {
		return nil, err
	}
	s.graphDriver = driver
...
func (s *store) GraphDriver() (context.Driver, error) {
	s.graphLock.Lock()
...
func (s *store) mount(id string, options context.MountOpts) (string, error) {
	rlstore, err := s.LayerStore()
	if err != nil {
		return "", err
	}
...
func (s *store) ApplyDiffFromStagingDirectory(to, stagingDirectory string, diffOutput *context.DriverWithDifferOutput, options *context.ApplyDiffOpts) error {
	rlstore, err := s.LayerStore()
	if err != nil {
		return err
	}
...
func (s *store) ApplyDiffWithDiffer(to string, options *context.ApplyDiffOpts, differ context.Differ) (*context.DriverWithDifferOutput, error) {
	rlstore, err := s.LayerStore()
	if err != nil {
		return nil, err
	}
...
func (s *store) LookupAdditionalLayer(d digest.Digest, imageref string) (AdditionalLayer, error) {
	adriver, ok := s.graphDriver.(context.AdditionalLayerStoreDriver)
	if !ok {
		return nil, ErrLayerUnknown
	}
	...
	if err != nil {
		if errors.Is(err, context.ErrLayerUnknown) {
			return nil, ErrLayerUnknown
		}
		return nil, err
	}
...
type additionalLayer struct {
	layer   *Layer
	handler context.AdditionalLayer
	s       *store
}
